...
export default async function CourseFeedbackPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/provider/courses" className="p-2 hover:bg-slate-100 rounded-full transition-colors">
          <ArrowLeft className="h-5 w-5 text-slate-500" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Trainee Feedback</h1>
      ...
      {feedbacks.length === 0 ? (
        <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
          <p className="text-slate-500">No feedback submitted yet for this course.</p>
        </div>
      ) : (
